...
namespace InventoryManagementSystem
{
    public partial class OrderModuleForm : Form
    {
        ...
        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("На складе нет такого количества!", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                UDQty.Value = UDQty.Value - 1;
                return;
            }
            if (Convert.ToInt32(UDQty.Value) > 0)
            {
                double total = Convert.ToDouble(txtPrice.Text) * Convert.ToInt32(UDQty.Value);
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtPid.Text == "")
                {
                    MessageBox.Show("Пожалуйста, выберите товар!", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (MessageBox.Show("Вы уверены, что хотите сделать этот заказ?", "Сохранить запись", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
